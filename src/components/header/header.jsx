...
export default function Header({
  ...
  // Refs for desktop popover outside-click
  ...
  useEffect(() => {
    if (!showPalette || isMobile) return;
    const onDown = (e) => {
      const w = desktopWrapRef.current;
      const p = desktopPopoverRef.current;
      if (!w || !p) return;
      if (!w.contains(e.target) && !p.contains(e.target)) setShowPalette(false);
    };
    const onKey = (e) => e.key === "Escape" && setShowPalette(false);
  ...
  return (
    <div className="header">
      ...
      <div className="header-controls">
        ...
        {showWorkspaceMenu && (
          <div className="theme-popover-wrap" ref={desktopWrapRef}>
            <button className="btn" onClick={() => setShowPalette((v) => !v)}>
              Theme
            </button>
            {!isMobile && showPalette && (
              <div className="palette-popover" ref={desktopPopoverRef}>
        ...
        <div className="palette-sheet-overlay" onClick={() => setShowPalette(false)}>
          <div className="palette-sheet" onClick={(e) => e.stopPropagation()}>
            <div className="palette-sheet-header">
              ...
              <button className="palette-sheet-close" onClick={() => setShowPalette(false)} aria-label="Close">
